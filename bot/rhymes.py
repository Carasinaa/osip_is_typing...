...
pairs = [['б', 'п'], ['в', 'ф'], ['г', 'к'], ['з', 'с'], ['д', 'т'], ['ж', 'ш'], ['ж', 'шь']]
suitable_words = []
    ...
    if len(syl_1) == 2 and len(syl_2) == 2:
        ...
        if check_pair(syl_1[-1], syl_2[-1]) == 'True':
            if syl_1[-2] and syl_2[-2] in vowels and syl_1[-2] == syl_2[-2]:
                return 'True'
    if len(syl_1) > 2 and len(syl_2) > 2:
        if syl_1[-1] in vowels and syl_2[-1] in vowels:
            if syl_1[-1] == syl_2[-1]:
                if syl_1[-2] == syl_2[-2] and syl_1[-2] in vowels and syl_2[-2] in vowels:
                    if syl_1[-3] == syl_2[-3]:
                        return 'True'
                if syl_1[-2] == syl_2[-2] and syl_1[-2] in vowels and syl_2[-2] in vowels:
                    return 'True'
        if syl_1[-1] not in vowels and syl_2[-1] not in vowels:
            if check_pair(syl_1[-1], syl_2[-1]) == 'True' or syl_1[-1] == syl_2[-1]:
                if syl_1[-2] in vowels and syl_2[-2] in vowels and syl_1[-2] == syl_2[-2]:
                    return 'True'
    else:
        return 'False'
...
def rhymes(last_syl_1, num_syl_1, last_syl_2, num_syl_2):
    if num_syl_1 == 1 and num_syl_2 == 1:
        ...
        if last_syl_1 == last_syl_2:
            return 'True'
        elif syls(last_syl_1, last_syl_2) == 'True':
            return 'True'
